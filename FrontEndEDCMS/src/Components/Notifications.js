...
export default function Notifications() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (!token) return
      try {
        const res = await fetch(`${API_BASE}/api/Business/employees/pending`, { headers: getHeaders() })
        if (!res.ok) return
  ...
  useEffect(() => {
    ...
    connection.on("ReceiveNotification", (note) => {
      setNotifications((prev) => [{ id: note.Id, message: note.Message, createdAt: note.CreatedAt }, ...prev])
    })
  ...
  return (
    <div ref={wrapperRef} className="notification-wrapper">
      ...
      {/* The panel is always rendered, its visibility is controlled by CSS classes */}
      <div id="notifications-panel" className={`notifications-panel ${open ? "open" : ""}`} role="menu">
            ...
            {invites.map((inv) => (
              <div key={inv.businessId} className="invitation-item">
                <div className="invitation-content">
                  ...
                  <div className="invitation-time">{new Date(inv.invitedAt).toLocaleString()}</div>
                </div>
                <div>
                  <button onClick={() => respondInvite(inv.businessId, true)} className="invite-action-btn">
                    Accept
                  </button>
                  <button onClick={() => respondInvite(inv.businessId, false)} className="invite-action-btn">
                    Decline
                  </button>
                </div>
              </div>
        ...
        {invites.length === 0 && notifications.length === 0 && <div className="no-items">No New Notifications</div>}
      </div>
    </div>
  )
}
