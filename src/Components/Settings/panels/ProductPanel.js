...
export default function ProductPanel({ business }) {
  ...
  const { role, userInfo } = useAuth()            // ✅ single useAuth
  ...
  const saveProduct = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.message ? `Failed to save product: ${err.message}` : "Failed to save product. Please check console/network for details.")
    } finally { setLoading(false) }
  }, [businessId, validateForm, form, mainPhotoIndex, editingId, role, post, put, resetForm])

  const handleDelete = useCallback(async (id) => {
    if (!businessId) return
    const confirmMessage = role === "employee" ? "Submit delete request for approval?" : "Permanently delete this product?"
    ...
    try {
      ...
      setError(err?.message ? `Failed to delete product: ${err.message}` : "Failed to delete product. Please check console/network for details.")
    } finally { setLoading(false) }
  ...
  const handleSale = useCallback(async (product) => {
    ...
    try {
      ...
      await post(`/api/Sales/ClothingItem/${product.clothingItemId}/sale`, {
        ...
        soldByName: seller,                      // ✅ snapshot of seller name
      })
  ...
  return (
    <>
      ...
      <div className="panel product-form">
        <h3><span className="panel-icon">📦</span>{editingId ? "Edit Product" : "Add New Product"}</h3>
        ...
        <div className="form-group">
          <div className="grid two-cols">
            ...
            <div className="form-group">
              ...
              <input id="product-brand" placeholder="Enter brand name"
                value={form.brand} onChange={(e) => setForm((f) => ({ ...f, brand: e.target.value }))} disabled={loading} />
            </div>
          </div>
        </div>
        ...
        <div className="form-group">
          <div className="grid two-cols">
            <div className="form-group">
              ...
              <input id="product-model" placeholder="Enter model"
                value={form.model} onChange={(e) => setForm((f) => ({ ...f, model: e.target.value }))} disabled={loading} />
            </div>
            <div className="form-group">
              <label htmlFor="product-price">Price in LEK *</label>
              <input id="product-price" type="number" step="0.01" min="0" placeholder="0.00"
                value={form.price} onChange={(e) => setForm((f) => ({ ...f, price: e.target.value }))} disabled={loading} />
            </div>
          </div>
        </div>
        ...
        <div className="form-group">
          ...
          <textarea id="product-description" placeholder="Enter product description"
            value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} disabled={loading} />
        </div>
        ...
        <div className="form-group">
          <div className="grid three-cols">
            <div className="form-group">
              ...
              <input id="product-quantity" type="number" min="0" placeholder="0"
                value={form.quantity} onChange={(e) => setForm((f) => ({ ...f, quantity: e.target.value }))} disabled={loading} />
            </div>
            <div className="form-group">
              <label htmlFor="product-category">Category</label>
              <select id="product-category" value={form.categoryId}
                onChange={(e) => setForm((f) => ({ ...f, categoryId: e.target.value }))} disabled={loading}>
            ...
            <div className="form-group">
              ...
              <input id="product-size" placeholder="e.g. M, 32W, One-Size"
                value={form.size} onChange={(e) => setForm((f) => ({ ...f, size: e.target.value }))} disabled={loading} />
            </div>
          </div>
        </div>
        ...
        <div className="form-group">
          <div className="grid two-cols">
            <div className="form-group">
              ...
              <input id="product-colors" placeholder="Red, Blue, Green (comma separated)"
                value={form.colors} onChange={(e) => setForm((f) => ({ ...f, colors: e.target.value }))} disabled={loading} />
            </div>
            <div className="form-group">
              <label htmlFor="product-material">Material</label>
              <input id="product-material" placeholder="Cotton, Polyester, etc."
                value={form.material} onChange={(e) => setForm((f) => ({ ...f, material: e.target.value }))} disabled={loading} />
            </div>
          </div>
        </div>
        ...
        <div className="form-group">
          ...
          <label className="file-btn" disabled={uploading || loading}>
            ...
            <input ref={fileInputRef} type="file" accept="image/*" multiple
              onChange={(e) => handleFileUpload(e.target.files)} disabled={uploading || loading} />
          </label>

          <div className={`photo-row ${dragOver ? "drag-over" : ""}`}
            ...
            {form.photos.map((url, index) => (
              <div key={`${url}-${index}`}
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); removePhoto(index) }}
                        disabled={loading} title="Remove photo">×</button>
          ...
          {form.photos.length > 0 && (
            <p className="photo-help">Click a photo to set it as the main image. Max 10 photos.</p>
          )}
        </div>
        ...
        <div className="actions">
          ...
          {editingId && (
            <button onClick={resetForm} disabled={loading} className="secondary">Cancel</button>
          )}
        </div>
      </div>
      ...
      <div className="panel product-list">
        <h3><span className="panel-icon">📋</span>Products ({totalCount})</h3>
              ...
              {paginatedProducts.length > 0 ? (
                paginatedProducts.map((p) => (
                  <li key={p.clothingItemId}>
                    ...
                    <div className="btns">
                      <button onClick={() => startEdit(p)} disabled={loading} className="edit">✏️ Edit</button>
                      <button onClick={() => handleDelete(p.clothingItemId)} disabled={loading} className="delete">🗑️ Delete</button>
                    ...
                    title="Record a sale (reduce stock by 1)"
                    style={{
                  ...
                  >
                    {saleLoadingId === p.clothingItemId ? "Selling…" : "Sale"}
                  </button>
                    </div>
                  </li>
                ))
