    ...
    public class ClothingItemController : ControllerBase
    {
        ...
        public async Task<ActionResult<ClothingItemDTO>> Post([FromBody] ClothingItemDTO dto)
        {
            ...
            try
            {
                ...
                var createdBizId = dto.BusinessIds.FirstOrDefault();
                await _notificationService.NotifyAsync(
                    userId,
                    $"Item ({dto.Name}) was created: price {dto.Price:C}, quantity {dto.Quantity}.",
                    createdBizId
                );
        ...
        public async Task<IActionResult> Put(int id, [FromBody] ClothingItemDTO dto)
        {
            ...
            try
            {
                var changeMsg = await _service.UpdateClothingItemAsync(id, dto, userId);
                await _notificationService.NotifyAsync(userId, changeMsg, bizId);
                return NoContent();
            }
            catch (Exception ex)
            {
                return StatusCode(500, new
                {
                    message = "Error during item update",
                    error   = ex.Message,
                    inner   = ex.InnerException?.Message
                });
            }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                ...
                await _notificationService.NotifyAsync(
                    userId,
                    $"Item ({existing.Name}) was deleted.",
                    bizId
                );
                return NoContent();
            }
            catch (Exception ex)
            {
                return StatusCode(500, new
                {
                    message = "Error during item deletion",
                    error   = ex.Message,
                    inner   = ex.InnerException?.Message
                });
            }
