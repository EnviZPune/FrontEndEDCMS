...
namespace EDClothingMS.Controllers
{
    ...
    public class BusinessController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create(
            [FromBody] CreateBusinessRequest req,
            [FromQuery] string token)
        {
            ...
            if (!await _payments.HasActiveSubscriptionAsync(userId))
                return Forbid("You need an active subscription to register a shop.");

            // 2) validate one-time token
            var oneTime = await _db.OneTimeTokens
                .SingleOrDefaultAsync(t => t.Token == token && t.UserId == userId);
            if (oneTime == null || oneTime.ExpiresAt < DateTime.UtcNow)
                return BadRequest("Registration link invalid or expired.");

            // 3) consume token
            _db.OneTimeTokens.Remove(oneTime);
            ...
            // 5) add owner as approved employee
